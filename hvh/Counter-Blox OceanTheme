...
local Window = Library.CreateLib("t.me/amyr_softikk", "Ocean")
...
FOVSection:NewSlider("Min FOV", "Стандартный FOV (70-120)", 250, 180, FOV_MIN, function(value)
    FOV_MIN = value
    ...
    AntiAimConnection = RunService.Heartbeat:Connect(function()
        ...
        if shouldRotate then
            ...
            if AntiAimSettings.AntiResolver then
                yawOffset = yawOffset + math.random(-20, 20)  -- Small randomization for desync
            end
...
OtherTabCategoryMain:NewToggle("No Chat Filter", "No Chat Filter", function(val)
    if val then
        SafeCall(function()

import os, sys, time, math, struct, json, random, tempfile
...
# Пример чтения
import json
with open(resource_path(".vs/ProjectSettings.json"), 'r') as f:
...
def create_window():
    ...
    window = glfw.create_window(WINDOW_WIDTH, WINDOW_HEIGHT, "ProjectX", None, None)
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".ttf") as weapon_font_file:
        weapon_font_file.write(weapon_bytes)
        weapon_font_path = weapon_font_file.name
        font_paths.append(weapon_font_path)
    weapon_font = io.fonts.add_font_from_file_ttf(weapon_font_path, weapon_size)

    with tempfile.NamedTemporaryFile(delete=False, suffix=".ttf") as verdana_font_file:
        verdana_font_file.write(verdana_bytes)
        verdana_font_path = verdana_font_file.name
        font_paths.append(verdana_font_path)
    ...
    verdana_font = io.fonts.add_font_from_file_ttf(verdana_font_path, font_size, font_config=config, glyph_ranges=ranges)
...
def render_loop(draw_func):
    ...
    try:
        while not glfw.window_should_close(window):
            ...
            draw_text(draw_list, 10, 10, f"FPS: {int(fps)}", (255, 255, 255, 255))
...
color_accent = (0.25, 0.55, 1.00, 1.00)  # Неоновый сине-фиолетовый для акцентов
color_bg_dark = (0.05, 0.05, 0.12, 0.85)  # Полупрозрачный темно-синий фон
color_item_bg = (0.10, 0.12, 0.16, 0.90)  # Полупрозрачный фон элементов
color_text = (0.95, 0.95, 1.00, 1.00)    # Яркий белый текст с голубым оттенком
color_border = (0.20, 0.25, 0.35, 0.40)   # Полупрозрачная граница
color_watermark_text = (0.25, 0.55, 1.00, 0.70)  # Полупрозрачный сине-фиолетовый для водяного знака
color_watermark_title = (0.30, 0.60, 1.00, 1.00)  # Полностью непрозрачный акцент для названия

def setup_imgui_style():
    global color_accent, color_bg_dark, color_item_bg, color_text, color_border
    style = imgui.get_style()
    style.window_rounding = 5  # Плавные углы
    style.frame_rounding = 5
    style.grab_rounding = 5
    style.scrollbar_rounding = 5
    style.window_padding = (10, 10)  # Просторные отступы
    style.frame_padding = (8, 5)
    style.item_spacing = (10, 8)
    style.scrollbar_size = 12  # Более заметная полоса прокрутки
    ...
    colors[imgui.COLOR_FRAME_BACKGROUND_HOVERED] = (0.15, 0.20, 0.25, 0.80)  # Полупрозрачный при наведении
    colors[imgui.COLOR_FRAME_BACKGROUND_ACTIVE] = (0.20, 0.25, 0.30, 0.85)  # Полупрозрачный активный
    colors[imgui.COLOR_CHECK_MARK] = color_accent
    colors[imgui.COLOR_SLIDER_GRAB] = color_accent
    colors[imgui.COLOR_SLIDER_GRAB_ACTIVE] = (0.35, 0.65, 1.00, 0.95)  # Легкая прозрачность
    colors[imgui.COLOR_HEADER] = color_accent
    colors[imgui.COLOR_HEADER_HOVERED] = (0.30, 0.60, 1.00, 0.90)  # Полупрозрачный при наведении
    colors[imgui.COLOR_HEADER_ACTIVE] = (0.20, 0.50, 0.90, 0.95)   # Полупрозрачный активный
    colors[imgui.COLOR_SCROLLBAR_BACKGROUND] = (0.08, 0.10, 0.15, 0.80)  # Полупрозрачный фон полосы прокрутки
    colors[imgui.COLOR_SCROLLBAR_GRAB] = (0.15, 0.20, 0.25, 0.90)  # Полупрозрачный ползунок прокрутки
    colors[imgui.COLOR_TEXT] = color_text
    colors[imgui.COLOR_TEXT_DISABLED] = (0.60, 0.60, 0.70, 0.80)  # Полупрозрачный отключенный текст
    colors[imgui.COLOR_BUTTON] = color_accent
    colors[imgui.COLOR_BUTTON_HOVERED] = (0.30, 0.60, 1.00, 0.90)  # Полупрозрачный при наведении
    colors[imgui.COLOR_BUTTON_ACTIVE] = (0.20, 0.50, 0.90, 0.95)   # Полупрозрачный активный
            ...
            try:
                ...
                if client_m:
                    ...
                    if entity_team != local_player_team or aim_attack_all:
                        ...
                        head_pos, leg_pos, deltaZ = esp_aim(self.pm, entity_pawn_addr, view_matrix, WINDOW_WIDTH, WINDOW_HEIGHT)
...
class Settings:
    def __init__(self, manager):
        ...
        self._data = manager.dict({
            ...
            "aimbot_enable": False, "aimbot_key": "MOUSE5", "aimbot_bone": 0, "aim_attack_all": False, "draw_fov": True,
            ...
            "bunnyhop_enable": True, "bunnyhop_key": "SPACE", "noflash_enable": False, "noflash_strength": 1.0, "radar_enable": True, "draw_crosshair": True,
    ...
    with Manager() as manager:
        ...
        if wait_cs2():
            ...
            for process in processes:
                ...
                time.sleep(10)  # Даст 10 секунд на копирование